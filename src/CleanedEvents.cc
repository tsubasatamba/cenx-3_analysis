...
void CleanedEvents::process(const std::string& filename)
{
  readFits(filename);
  correctData();
  writeFits(filename);
...
void CleanedEvents::readFits(const std::string& filename)
{
...
void CleanedEvents::correctData()
{
  correctTime(tstart_);
  correctTime(tstop_);
...
void CleanedEvents::writeFits(const std::string& filename)
{
  cfitsio::fitsfile* fitsfile = nullptr;
  int fits_status = 0;
  std::vector<std::string> keywords = {"TSTART", "TSTOP", "DATE-OBS", "DATE-END"};  
...
void CleanedEvents::correctTime(double& t)
{
  DetectedEvent ev;
  ev.setArrivalTime(t);
